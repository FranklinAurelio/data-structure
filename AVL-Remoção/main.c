...
//  AVL-Remoção
//
//  Created by Franklin Carvalho on 21/07/21.
//
...
int AlturaRubro(struct Node* No)
{
    int esq, dir;
    if(No == NULL)
        return(0);
    else {
        esq = AlturaRubro(No->left); //Conta a altura rubro da esquerda
        dir = AlturaRubro(No->right); //Conta a altura rubro da direita
    }
 
    if(esq > dir)
        return(EhRubro(No)+esq); //Retorna somente o número de nós rubros
    else
        return(EhRubro(No)+dir);
}
